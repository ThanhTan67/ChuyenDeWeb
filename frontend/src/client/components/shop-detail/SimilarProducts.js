...
const SimilarProducts = ({ productId }) => {
    ...
    const [scores, setScores]     = useState([]);   // content similarity scores
        ...
        (async () => {
            ...
            try {
                const res = await fetch(
                    `${API_BASE_URL}/api/recommendations/similar/${productId}?topN=8`,
                    { credentials: 'include' }
                );
                ...
                const sliced = data.products.slice(0, 8);
                const rawScores = sliced.map(p => p.finalScore ?? p.final_score ?? null);

                const settled = await Promise.allSettled(
                    sliced.map(item =>
                        fetch(`${API_BASE_URL}/api/products/${item.product_id ?? item.productId}`, {
                            credentials: 'include',
                ...
                settled.forEach((r, i) => {
                    if (r.status === 'fulfilled' && r.value && r.value.id !== Number(productId)) {
                        resolvedProducts.push(r.value);
                        resolvedScores.push(rawScores[i]);
                    }
                });
    ...
    const scrollByCards = (dir) => {
        ...
        const step = card ? card.offsetWidth + 18 : 233;
        ...
        <section className="rec_section" ref={sectionRef}>
            <div className="container">
                ...
                <div className="rec-header">
                    <div className="rec-header__badge strategy--hybrid">
                        <span>🔗</span> Tương tự
                    </div>
                    <h2 className="rec-header__title">
                        Sản phẩm tương tự <span className="rec-header__underline" />
                    </h2>
                    <p className="rec-header__sub">
                        Được AI gợi ý dựa trên nội dung và đặc điểm của sản phẩm này
                    </p>
                </div>
                ...
                <div className="rec-carousel-wrap">
                    ...
                    <div className="rec-track" ref={trackRef} onScroll={updateArrows}>
                        {loading
                            ...
                            : products.map((product, idx) => {
                                ...
                                // Convert similarity score (0-1) to percent
                                const simPct = score !== null && score !== undefined
                                ...
                                return (
                                    ...
                                    >
                                        {/* Strategy chip */}
                                        <span className="rec-chip strategy--hybrid">
                                            🔗 Tương tự
                                        </span>
                                        ...
                                        >
                                            ...
                                            {simPct !== null && (
                                                <div className="rec-match-bar" title={`Độ tương đồng: ${simPct}%`}>
                                                    <div className="rec-match-label">
                                                        <span className="rec-match-label__text">✦ Độ tương đồng</span>
                                                        <span className="rec-match-label__pct">{simPct}%</span>
                                                    </div>
                                                    <div className="rec-match-track">
                                                        <div className="rec-match-fill" style={{ width: `${simPct}%` }} />
                                                    </div>
                                                </div>
                                            )}
                                        </div>
