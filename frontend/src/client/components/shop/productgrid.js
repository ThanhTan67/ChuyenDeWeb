...
// Biến môi trường chung (fallback localhost cho dev)
...
const ProductGrid = ({ searchTerm = '', sortBy = 'name', sortOrder = 'asc', category = '', brand = '' }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            setError(null);
            try {
                let url = `${PRODUCTS_API}/grid?page=${currentPage - 1}&size=${itemsPerPage}`;

                if (searchTerm.trim()) {
                    url = `${PRODUCTS_API}/search?keyword=${encodeURIComponent(searchTerm)}&page=${currentPage - 1}&size=${itemsPerPage}`;
                }
                if (sortBy !== 'name' || sortOrder !== 'asc') {
                    url = `${PRODUCTS_API}/sorted?keyword=${encodeURIComponent(searchTerm || '')}&page=${currentPage - 1}&size=${itemsPerPage}&sortBy=${sortBy}&sortOrder=${sortOrder}`;
                }
                if (category || brand) {
                    url = `${PRODUCTS_API}/filter?keyword=${encodeURIComponent(searchTerm || '')}&category=${encodeURIComponent(category || '')}&brand=${encodeURIComponent(brand || '')}&page=${currentPage - 1}&size=${itemsPerPage}&sortBy=${sortBy}&sortOrder=${sortOrder}`;
                }
    ...
    const handleAddToCartFromModal = async () => {
        ...
        try {
            ...
            fetchCart();
            setTimeout(() => setSuccessModalOpen(false), 1300);
        } catch (err) {
